...
This script creates a test set by copying a percentage of images from each class
...
def parse_args():
    ...
    parser.add_argument('source_dir',
                        help='Source directory containing training images (e.g., leaves/images/)')
    parser.add_argument('test_dir',
                        help='Destination directory for test images')
    parser.add_argument('--test-size', type=float, default=0.15,
                        help='Fraction of images to use for testing (default: 0.15)')
    parser.add_argument('--min-test-per-class', type=int, default=15,
                        help='Minimum number of test images per class (default: 15)')
    parser.add_argument('--seed', type=int, default=42,
                        help='Random seed for reproducibility (default: 42)')
    parser.add_argument('--copy', action='store_true',
                        help='Copy files instead of moving them (keeps original data intact)')
...
def create_test_split(source_dir, test_dir, test_size=0.15, min_test_per_class=15, 
                      seed=42, copy_mode=False):
    """
    ...
    if total_test < 100:
        print(f"⚠ WARNING: Test set has only {total_test} images (minimum requirement: 100)")
        print("Consider using a larger test_size or ensuring enough images in source directory.\n")
    else:
        print(f"✓ Test set has {total_test} images (meets minimum requirement of 100)\n")
